...
def adapter_catalog(tools: dict[str, bool] | None = None) -> dict[str, Any]:
    # Contract: tools=None probes the host (auto-detect); an explicit dict —
    # even empty — is authoritative, so callers get reproducible answers and
    # tests stay hermetic across machines.
    auto_detect = tools is None
    tools = tools or {}
    ...
    for name, adapter in load_adapters().items():
        available_commands = [
            ...
            if tools.get(command) or (auto_detect and shutil.which(command))
        ]
