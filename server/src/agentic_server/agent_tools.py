...
from agentic_server.local_tools import detect_environment, run_bash, run_bash_stream
from agentic_server.app_capabilities import build_app_capability_contract
from agentic_server.artifact_kernel import evaluate_artifact_write
from agentic_server.design_intent import evaluate_write_against_intent, intent_from_state_or_manifest
...
from agentic_server.vlsi_capability_graph import assess_design_readiness, bind_memory_requirement, compact_capability_graph
...
def parse_module_tool(path: str, workspace_root: str, module: str = "") -> str:
    full = _safe_workspace_path(path, workspace_root)
    ...
    return json.dumps({
        "requested_module": module or None,
        "ports": ports,
...
def schematic_json_tool(path: str, workspace_root: str, module: str = "") -> str:
    ...
    if res.refused:
        return json.dumps({"available": False, "reason": res.refused})

    if res.code != 0 or not os.path.isfile(cache_path):
...
def lint_file_tool(path: str, workspace_root: str) -> str:
    ...
    if vlogan_tool:
        try:
            res = exec_service.run([vlogan_tool, "-sverilog", "-q", full], timeout=10)
            output = res.combined
            current_error = None
        ...
        try:
            res = exec_service.run([xmvlog_tool, "-sv", "-messages", full], timeout=10)
            output = res.combined
            for line in output.splitlines():
        ...
        try:
            res = exec_service.run([vlog_tool, "-sv", "-lint", full], timeout=10)
            output = res.combined
            for line in output.splitlines():
        ...
        try:
            ...
            for sub in ["rtl", "simulation", "verification"]:
                ...
                if os.path.isdir(subdir):
                    ...
                    cmd.extend(["-y", subdir])
            cmd.append(full)
            res = exec_service.run(cmd, timeout=10)
            output = res.combined
            for line in output.splitlines():
...
def git_clone(url: str, workspace_root: str, target_dir: str | None = None, branch: str = "main", token: str = "") -> str:
    ...
    try:
        ...
        res = exec_service.run(cmd, timeout=120)
        if res.timed_out:
...
def dispatch_tool(name: str, args: dict, workspace_root: str, design_name: str = "scratch", on_output=None, cancel_checker=None) -> str:
    if name == "workspace":
        ...
        from agentic_server.silicon_compiler_adapter import get_sc_adapter
...
def _rtl_syntax_preflight(path: str, content: str) -> str:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="agentic_rtl_lint_") as tmp:
            ...
            res = exec_service.run([tool, "--lint-only", tmp_path], timeout=20)
            if res.ok:
                return ""
            return "Error: " + json.dumps({
                ...
                "tool": "verilator",
                "exit_code": res.code,
                "log_snippet": res.combined.strip()[:3000],
            }, indent=2)
    except Exception:
        return ""
