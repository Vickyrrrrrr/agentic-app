...
function wireWindowRecovery(win: BrowserWindow, name: string) {
  ...
  win.webContents.on("render-process-gone", (_event, details) => {
    ...
    void show(
      "AgentIC window terminated unexpectedly",
  ...
  win.on("unresponsive", () => {
    ...
    sampler.start()
    void show("AgentIC is not responding", "You can relaunch the app, open the logs, or keep waiting.", true)
  })
  win.on("responsive", () => {
